      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-separate border-spacing-0 text-sm tabular-nums">
          <thead className="sticky top-0 z-10 bg-slate-100 shadow-[0_1px_0_0_rgb(203_213_225)] dark:bg-slate-800 dark:shadow-[0_1px_0_0_rgb(51_65_85)]">
            <tr>
              {selectable ? (
                <th scope="col" className="w-10 border-r border-slate-200 px-3 py-2.5 dark:border-slate-700">
                  <input
                    type="checkbox"
              ...
              {visibleColumns.map((column) => {
                ...
                return (
                  <th
                    ...
                    scope="col"
                    className={cn(
                      "whitespace-nowrap border-r border-slate-200 px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-slate-600 last:border-r-0 dark:border-slate-700 dark:text-slate-300",
              ...
              {renderActions ? (
                <th
                  scope="col"
                  className="w-40 whitespace-nowrap px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300"
                >
                  Ações
                </th>
              ) : null}
            ...
            {loading ? (
              Array.from({ length: 5 }).map((_, index) => (
                <tr key={`skeleton-${index}`}>
                  {Array.from({ length: columnCount }).map((__, cellIndex) => (
                    <td key={`skeleton-cell-${cellIndex}`} className="border-b border-r border-slate-200 px-3 py-3 last:border-r-0 dark:border-slate-800">
              ...
              rows.map((row) => {
                ...
                return (
                  ...
                  >
                    {selectable ? (
                      <td className="border-b border-r border-slate-200 px-3 py-2.5 dark:border-slate-800">
                        <input
                          type="checkbox"
                    ...
                    {visibleColumns.map((column) => (
                      <td
                        key={column.id}
                        className={cn(
                          "border-b border-r border-slate-200 px-3 py-2.5 text-slate-700 last:border-r-0 dark:border-slate-800 dark:text-slate-300",
                          alignClasses[column.align ?? "left"],
                    ...
                    {renderActions ? (
                      <td className="border-b border-slate-200 px-3 py-2.5 text-right dark:border-slate-800">
                        {renderActions(row)}
                      </td>
                    ) : null}
                  </tr>
