...
function ActivitiesList({ onOpen }: { onOpen: (s: ActivitySheet) => void }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Activities"
        description="Each sheet groups trips for the same client, route, and registration date. Open a sheet to follow the progress reported by drivers."
        secondaryActions={
          <ActionButton
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 bg-white px-6 py-14 text-center dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">
            There are no activities yet. Add trips on the Operational Board.
          </p>
        </div>
      ) : (
...
function SheetTracking({
  ...
  function exportExcel() {
    exportToCsv(
      `activity-sheet-${sheet.clientName}-${sheet.day}.csv`,
      rows,
  ...
  async function exportPdf() {
    ...
    await addPdfHeader(pdf, "Operational Board", sheetTitle);
    ...
    addPdfFooter(pdf, `${sheet.total} trips · generated by SGRTC`);
    pdf.save(`activity-sheet-${sheet.clientName}-${sheet.day}.pdf`);
  }
  ...
  function advance(trip: Trip) {
    ...
    updateStatus.mutate(
      { id: trip.id, payload: { currentStatus: next } },
      {
        onSuccess: () => {
          toast({
            title: `Status → ${tripStatusMeta[next].label}`,
            type: "success",
          });
          void refetch();
        },
        onError: (error) =>
          toast({
            title: "Could not advance the trip status",
  ...
  function confirmDelete() {
    ...
    deleteTrip.mutate(deletingTrip.id, {
      onSuccess: () => {
        toast({ title: "Trip deleted", type: "success" });
  ...
  const headers = canManage ? [...HEADERS, "Actions"] : HEADERS;
  ...
  return (
    <div className="flex min-h-0 flex-col gap-4">
      ...
      >
        <ArrowLeft className="size-4" aria-hidden /> Activities
      </button>
      <PageHeader
        title={`${sheet.clientName} · ${sheet.origin} → ${sheet.destination}`}
        description={`Sheet for ${formatDate(sheet.day)} · ${sheet.total} trips · tracking and trip management`}
        showIdentity
        secondaryActions={
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {canEditState && !terminal && next ? (
                          <button
                            ...
                            title={`Advance to ${tripStatusMeta[next].label}`}
      ...
      <ConfirmDialog
        ...
        description={
          deletingTrip
            ? `The trip for ${deletingTrip.bookingReference ?? deletingTrip.cargo.code} will be cancelled and removed, releasing its assigned resources. This action cannot be undone.`
            : undefined
        }
        confirmLabel="Delete"
